...
def loadModel(weightsfile):
    ...
    while True:
        try:
            v, p = pickle.load(fin, encoding='iso-8859-1')
        except EOFError:
            break
        loaddict[v] = p
    fin.close()
...
def run(img_path):
    model = loadModel('../../../Models/zyj/psgn/r2n2_128_v1.pkl')
    save_path = './temp/%s.xyz' % time.strftime('%Y-%m-%d-%H-%M-%S')
    out = open(save_path, 'w')
    img = cv2.imread(name)
    img = cv2.resize(img, (HEIGHT, WIDTH))
    ret = run_image(model, img)
    for x, y, z in ret:
        out.write('%f %f %f\n' % (x, y, z))
    out.close()
    return save_path
