...
# depth only to mesh
def depth2mesh(depth_src, pose_src, cam_intr, n_imgs):
    """the function is to convert depth map to mesh
    input:
        depth_src: the source of depth
        pose_src: the source of camera pose
        cam_intr: camera-intrinsics, 4*4 matrix
        n_imgs: number of frames, int
    output:
        verts: verts list
        faces: faces list
    """
    print("depth2mesh")
    # use the 3*3 camera intrinsics matirx
    cam_intr = cam_intr[:3, :3]
    # some undetermined parameters
    depth_const = 1000.0  # depth is saved in 16-bit PNG in millimeters

    vol_bnds = np.zeros((3, 2))

    for i in range(n_imgs):
        # Read depth image and camera pose
        depth_im = depth_src[i]
        depth_im.astype(np.float16)
        depth_im = depth_im / depth_const

        cam_pose = pose_src[i]  # 4x4 rigid transformation matrix
    ...
    tsdf_vol = TSDFVolume_depth(vol_bnds, voxel_size=0.02)

    # Loop through depth images and fuse them together
    for i in range(n_imgs):
...
# rgbd to mesh
def rgbd2mesh(depth_src, pose_src, color_src, cam_intr, n_imgs):
    """the function is to convert depth map to mesh
        input:
            depth_src: the source of depth
            pose_src: the source of camera pose
...
def meshwrite(verts, faces, colors=None, filename='mesh.ply'):
    if colors is None:
        meshwrite_uncolor(filename, verts, faces)
    else:
        meshwrite_color(filename, verts, faces, colors)


class TSDFVolume_depth:
    """Volumetric TSDF Fusion of Depth Images Only.
  """
    ...
    def integrate(self, depth_im, cam_intr, cam_pose, obs_weight=1.):
        """Integrate an Depth frame into the TSDF volume.
...
def meshwrite_uncolor(filename, verts, faces):
...
def meshwrite_color(filename, verts, faces, colors):
    """Save a 3D mesh to a polygon .ply file.
  """
    # Write header
    ply_file = open(filename, 'w')
